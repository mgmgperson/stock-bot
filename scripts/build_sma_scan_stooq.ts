...
const NEED = 210; // buffer above 200

function toStooqSymbol(sp500: string): string {
  // BRK.B -> brk-b.us, BF.B -> bf-b.us, etc.
...
async function main() {
  ...
  await mapPool(symbols, 6, async (sym) => {
    ...
    try {
      ...
      if (rows.length < 205) return null;

      // stooq is typically oldest->newest; use newest first for your SMA function
      const latest = rows[rows.length - 1];
      asOf = asOf || latest.date;
  ...
  // sort each list (most below first)
  for (const w of WINDOWS) {
  ...
  const outPath = path.join(process.cwd(), "public", "sma-scan.json");
  fs.writeFileSync(outPath, JSON.stringify(out, null, 2) + "\n", "utf8");
  console.log("Wrote:", outPath);
  console.log("Counts:", out.countByWindow);
}
